...
@Component({
  ...
  styleUrls: ['./unit-master.component.sass']
})
export class UnitMasterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    try {
      ...
      objWebErrorLogModel.ErrorPage = "UnitMaster";
    ...
    this.columnDefs = [
      ...
      { headerName: 'Unit Name ', field: 'UnitName', width: 550, resizable: true, filter: true, },
    ...
    this.GetUserPageRight(this.UnitNameId);
  }

  //brahamjot kaur 31/10/2022
  async GetUserPageRight(id:number) {
    this._Commonservices.GetUserPageRight(this.UserId, MenuName.UnitList).subscribe(data => {
      if (data.Status == 1) {
        console.log(data);
  ...
  CreateNew() {
    ...
    this.model.hiddenUnitId = null;
    this.model.hiddenUnitId = 0;
  ...
  AddUpdateUnitMasterDetial() {
    try {
      if (this.ValidationBasic() == 1) {
        return false;
      }
      var objItemNameModel = new ItemNameModel();
      objItemNameModel.Id = this.model.hiddenUnitId;
      objItemNameModel.UserId = this.UserId;
      objItemNameModel.ItemName = this.model.UnitName;
      objItemNameModel.Status = this.model.Isactive;
      objItemNameModel.Flag = "UnitMaster";
  ...
  SearchItemMasterGridList(para: string) {
    try {
      ...
      objpara.Flag = para;
      this._MasterService.GetAllUnitMasterList(objpara).subscribe(data => {
      ...
      objWebErrorLogModel.ErrorBy = this.UserId;
      objWebErrorLogModel.ErrorMsg = Error.message;
      objWebErrorLogModel.ErrorFunction = "GetAllUnitMasterList";
      objWebErrorLogModel.ErrorPage = "UnitMaster";
      this._GlobalErrorHandlerService.handleError(objWebErrorLogModel);
      //console.log(Error.message)
    }
  ...
  ShowItemMasterDeatil(e) {
    this.UnitNameId = e.rowData.UnitNameId;
    this.GetUserPageRight(this.UnitNameId);
    var UnitEditData = this.rowData.filter(
      m => m.UnitNameId === parseInt(e.rowData.UnitNameId));
    if (UnitEditData != null) {
      this.ConfirmmationClick();
      this.model.hiddenUnitId = UnitEditData[0].UnitNameId;
      this.model.UnitName = UnitEditData[0].UnitName;
      if (UnitEditData[0].Status == true) {
  ...
  ValidationBasic() {
    var flag = 0;
    if (this._Commonservices.checkUndefined(this.model.UnitName) == "") {
