...
import { CustomerToCustomerItemList, SaveUpDateCustomerToCustomerTransfer } from 'src/app/_Model/CustomerToCustomerModel';
import { CtcService } from 'src/app/Service/ctc.service';
...
@Component({
  ...
  styleUrls: ['./customer-to-customer.component.sass'],
  providers: [DatePipe]
})
export class CustomerToCustomerComponent implements OnInit {
  model: any = {};
  apiCSVIData: any = {};
  ...
  constructor(private datePipe: DatePipe, private _objSearchpanelService: SearchpanelService,
    ...
    private _PurchaseOrderService: PurchaseOrderService, private _MaterialMovementService: MaterialMovementService,
    private _GrncrnService: GrncrnService, private _CTCServices: CtcService) {
  ...
  ngOnInit(): void {
    ...
    this.model.WHId = "0";
    // this.model.RepairedType = "0"; // brahamjot kaur 30/8/2022
    this.model.InvoiceValue = 0.00;
    // this.model.VendorName = "0"; //brahamjot kaur 30/8/2022
    ...
    this.columnDefs = [
      ...
      { headerName: 'Old Customer Type', field: 'PreviousCustomer', width: 180 },
      { headerName: 'Equipment Type', field: 'EquepmentType', width: 180, sortable: true },
      ...
      { headerName: 'Unit Name', field: 'UnitName', width: 150 },
      { headerName: 'Qty', field: 'Qty', width: 150 },
      { headerName: 'New Customer Type', field: 'NewCustomer', width: 180 },
      { headerName: 'Created On', field: 'CreatedDate', width: 120 },
    ...
    // this.model.InCaseReason = "0"; //brahamjot kaur 30/8/2022
    // this.RepairedType();
    this.ItemReason();
    //brahamjot kaur 31/10/2022
    this.GetUserPageRight(this.CTCId);
  }

  //brahamjot kaur 31/10/2022
  async GetUserPageRight(id: number) {
    this._Commonservices.GetUserPageRight(this.UserId, MenuName.CustomerToCustomerTransfer).subscribe(data => {
      if (data.Status == 1) {
        console.log(data);
  ...
  CreateNew() {
    ...
    // this.model.RepairDate = ""; //brahamjot kaur 30/8/2022
    // this.FaultuRep_Id = 0;
    // this.model.VendorName = "0"; //brahamjot kaur 30/8/2022
  ...
  SaveUpDateCustomerToCustomerTransfer() {
    try {
      jQuery('#confirm').modal('hide');
      var objSaveUpdateCustomerToCustomerTransfer = new SaveUpDateCustomerToCustomerTransfer();
      objSaveUpdateCustomerToCustomerTransfer.CTCId = this.CTCId;
      objSaveUpdateCustomerToCustomerTransfer.UserId = this.UserId;
      objSaveUpdateCustomerToCustomerTransfer.CompanyId = this.CompanyId;
      objSaveUpdateCustomerToCustomerTransfer.WHId = this.model.WHId;
      objSaveUpdateCustomerToCustomerTransfer.Remarks = this.model.Remarks;
      this.CustomerItemList = [];
      for (var i = 0, len = this.dynamicArray.length; i < len; i++) {
        this.addBtn(i);
        var objCustomerToCustomerItemList = new CustomerToCustomerItemList();
        objCustomerToCustomerItemList.CTCListId = this.dynamicArray[i].Id;
        objCustomerToCustomerItemList.ItemCodeId = this.dynamicArray[i].ItemId;
        objCustomerToCustomerItemList.EqpTypeId = this.dynamicArray[i].EqTypeId;
        objCustomerToCustomerItemList.Remarks = this.dynamicArray[i].Remarks;
        objCustomerToCustomerItemList.UnitId = this.dynamicArray[i].UnitName;
        objCustomerToCustomerItemList.OldClientId = this.dynamicArray[i].ClientId;
        objCustomerToCustomerItemList.SerialNo = this.dynamicArray[i].SerialNo;
        objCustomerToCustomerItemList.NewClientId = this.dynamicArray[i].newClientId;
  ...
  SearchGetCustomerToCustomerList(para: string) {
    try {
      ...
      this._CTCServices.SearchGetCustomerToCustomerList(objpara).pipe(first()).subscribe(data => {
  ...
  ShowFaultyRepDetail(e) {
    ...
    this.SearchCustomerToCustomerEditById(e.rowData.CTCId);
  }
  SearchPdfPreview() {
  ...
  SearchCustomerToCustomerEditById(Id: any) {
    try {

      var objModel = new SaveUpDateCustomerToCustomerTransfer();
      objModel.CTCId = Id;
      this.CTCId = Id;
      this.GetUserPageRight(this.CTCId);
      this._CTCServices.GetCustomerToCustomerEditById(objModel).pipe(first()).subscribe(data => {
        if (data.Status == 1) {
          this.SerialNoList = [];
          ...
          for (var i = 0, len = this.ItemEditDataArr.length; i < len; i++) {
            ...
            if (this.ItemEditDataArr[i].OldClient_Id != "" && this.ItemEditDataArr[i].OldClient_Id != null) {
              objdynamic.ClientId = this.ItemEditDataArr[i].OldClient_Id;
            } else {
              objdynamic.ClientId = "0";
            }

            if (this.ItemEditDataArr[i].NewClient_Id != "" && this.ItemEditDataArr[i].NewClient_Id != null) {
              objdynamic.newClientId = this.ItemEditDataArr[i].NewClient_Id;
            } else {
              objdynamic.newClientId = "0";
            }
    ...
    try {
      ...
      this._StockserviceService.SearchFaultyRepPDFDataById(objModel).pipe(first()).subscribe(data => {
        if (data.Status == 1) {
          ...
          // this.model.RepairDate = data.Data[0].RepairDate; // brahamjot kaur 30/8/2022
  ...
  UpadateCancelDispatch() {
    try {
      ...
      // objApprovelStatusModel.Table_Id = this.model.InCaseReason; //brahamjot kaur 30/8/2022
  ...
  Validation() {
    ...
    // if(this.model.WHId == "" && this.model.WHId == "0"){
    //   this.submitted = false;
    // }
    ...
    for (var icount = 0, len = this.dynamicArray.length; icount < len; icount++) {
      ...
      //brahamjot kaur 1-sep
      if (this.dynamicArray[icount].newClientId == "" || this.dynamicArray[icount].newClientId == "0") {
        $('#ddlnewClient_' + icount).css('border-color', 'red');
        $('#ddlnewClient_' + icount).focus();
        flag = 1;
      }
      else {
        $("#ddlnewClient_" + icount).css('border-color', '');
      }

      if (this.dynamicArray[icount].ChangeQty == "" || this.dynamicArray[icount].ChangeQty == "0") {
        $('#txtChangeQty_' + icount).css('border-color', 'red');
        $('#txtChangeQty_' + icount).focus();
        flag = 1;

      }
      else {
        $("#txtChangeQty_" + icount).css('border-color', '');
      }
